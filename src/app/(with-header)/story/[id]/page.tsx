...
import { getStoryDetail, type ServerStoryDetail } from "@/lib/api/story/detail";
import {
  getComment,
  type ServerComment,
  postComment,
...
export default function StoryDetail({
  ...
  const onToggleLike = async () => {
    try {
      const res = await postLike(storyIdNum);
  ...
  // -------------------------- 상세 데이터 --------------------------
  const [apiItems, setApiItems] = useState<ServerStoryDetail | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 글 상세 정보 가져오기
        const res = await getStoryDetail(Number(id), accessToken || undefined);
        const detail = (res as any).result ?? res;
        setApiItems(detail as ServerStoryDetail);

        // 좋아요 초기값 동기화
        setLiked(!!detail.liked);
        setLikeCount(Number(detail.likeCount ?? 0));
      } catch (e: any) {
        console.error(e);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 댓글 첫 페이지
        const { result } = await getComment({ storyId: storyIdNum, size: 5 });
  ...
  const imageList = useMemo(() => {
    ...
    console.log("🖼️ [Story Detail] Images:", imageArray);
